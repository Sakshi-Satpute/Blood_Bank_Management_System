...
class DonorAppointment extends Component {
  constructor(props) {
    ...
    this.tentativeDateHandler = this.tentativeDateHandler.bind(this);
  }
  ...
  tentativeDateHandler = (event) => {
    ...
    this.tentativeDateValidation(newErrors, dateString)

  }

  tentativeDateValidation = (newErrors, dateString) => {
    let flag = true;
    if (dateString === '') {
      newErrors.tentativeDate = ' Cannot be blank!'
      flag = false;
    }
    else {
      var today = new Date();
      ...
      if(today > tDate){
        newErrors.tentativeDate = ' data can not be older!'
        flag = false;
      }
    }
    if (flag) {
  ...
  handleSubmit = (event) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      console.log(donor);
      axios.post('http://localhost:7070/donor/register', donor)
        .then(response => {
          console.log("componentDidMount");
  ...
  componentDidMount() {
    if (this.state.statesdata.length === 0) {
      ...
      //http://localhost:7070/commondata/cities/39
      axios
        .get("http://localhost:7070/commondata/allstates")
        .then((response) => {
          console.log("componentDidMount");
          console.log(response.data);
  ...
  render() {
    ...
    const {
      isValidFirstName, isValidLastName, isValidBirthDate, isValidTentativeDate, isValidGender,
          ...
          <div className="row">
            ...
            <div className="col-md-4 ">
              <Form.Group className="m-2" controlId="formBasicTentativeDate" name="TentativeDate" required>
                ...
                <Form.Control type="date" name="tentativeDate" onChange={this.tentativeDateHandler}
                  isValid={isValidTentativeDate} isInvalid={!!errors.tentativeDate} />
                <Form.Control.Feedback ><b>Looks good!</b></Form.Control.Feedback>
                <Form.Control.Feedback type="invalid"><b>{errors.tentativeDate}</b></Form.Control.Feedback>
              </Form.Group>
            </div>
            <div className="col-md-4">
              <Form.Group className="m-2">
